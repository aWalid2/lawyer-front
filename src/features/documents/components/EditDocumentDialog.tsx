...
export const EditDocumentDialog: React.FC<EditDocumentDialogProps> = ({
  ...
  const currentCaseValue = useMemo(() => {
    const caseValue =
      document.case_id || document.caseId || document.case?.id || "";
    return String(caseValue);
  }, [document.case?.id, document.caseId, document.case_id]);
  ...
  const caseOptions = useMemo(() => {
    ...
    if (
      currentCaseValue &&
      currentCaseLabel &&
      !caseEntries.has(currentCaseValue)
    ) {
      caseEntries.set(currentCaseValue, currentCaseLabel);
    }
  ...
  const handleSubmit = (values: EditDocumentFormValues) => {
        ...
        >
          {({ values }) => (
            <Form className="custom-scrollbar flex-1 space-y-4 overflow-y-auto pb-2 pl-2">
              ...
              >
                {isPending ? "جاري التعديل..." : "تعديل المستند"}
              </button>
            </Form>
          )}
        </Formik>
