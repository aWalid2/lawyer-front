...
import { useGetFirstInstanceSession } from "../api/hooks/useGetFirstInstanceSession";

interface FirstInstanceSessionDetailsDialogProps {
...
export const FirstInstanceSessionDetailsDialog: React.FC<
  FirstInstanceSessionDetailsDialogProps
> = ({ sessionId, open, onOpenChange, onEdit }) => {
  const {
    ...
    isError,
    error,
  } = useGetFirstInstanceSession(sessionId, open);

  return (
    <LayoutDialog
      title="تفاصيل جلسة أول درجة"
      open={open}
    ...
    >
      ...
      {isPending ? (
        ...
        <Error
          message="حدث خطأ أثناء جلب تفاصيل جلسة أول درجة."
          error={error}
        />
      ) : (
        <CustomLayoutBorder>
