...
const TypeBadge: React.FC<{ type: string }> = ({ type }) => {
  ...
  return (
    <span
      className={`inline-block rounded-full px-4 py-2 text-sm font-bold ${getTypeStyle(type)}`}
    >
      {type}
    </span>
  );
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <span
      className={`inline-block rounded-full px-4 py-2 text-sm font-bold ${getStatusStyle(status)}`}
    >
      {status}
    </span>
  );
...
export const LegislationDetails: React.FC = () => {
  ...
  const getTypeIcon = () => {
    switch (legislation.legislationType) {
      case "قانون":
        return <Scale className="h-8 w-8 text-[#153A4D]" />;
      case "لائحة":
        return <FileText className="h-8 w-8 text-[#CBA462]" />;
      case "قرار وزاري":
        return <BookOpen className="h-8 w-8 text-[#4B5675]" />;
      case "حكم محكمة":
        return <Gavel className="h-8 w-8 text-[#0B6E1F]" />;
      default:
    ...
    <PageLayout>
      ...
      <div className="mt-6 mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="mb-6 flex items-start justify-between">
          <div className="flex items-center gap-4">
            <div className="rounded-xl bg-gray-100 p-3">{getTypeIcon()}</div>
            <div>
              <h2 className="mb-2 text-2xl font-bold text-[#153A4D]">
      ...
      {legislation.summary && (
        <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h3 className="mb-4 text-lg font-bold text-[#153A4D]">ملخص</h3>
          <p className="leading-relaxed text-gray-700">{legislation.summary}</p>
        </div>
      )}
    </PageLayout>
  );
