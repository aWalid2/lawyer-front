...
const columns: Column<CaseData>[] = [
...

  {
    ...
    accessor: (item) => (
      <span
        className={`rounded-main px-3.5 py-1.5 text-xs font-medium whitespace-nowrap ${getProblemStyles(
          item.problem,
        )}`}
      >
        {item.problem}
...
export const TableIncomplete = () => {
  return <DataTable data={data} columns={columns} rowIdField="id" />;
};
